...
//#ifdef DOG_REVERSE
static const dog_pgm_uint8_t dog_mask7_reverse[8] = { 0xff, 0x7f, 0x3f, 0x1f, 0x0f, 0x07, 0x03, 0x01 };
static const dog_pgm_uint8_t dog_mask0_reverse[8] = { 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff };
//#else
static const dog_pgm_uint8_t dog_mask0_normal[8] = { 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f, 0xff };
...
uint8_t dog_get_vline_mask(uint8_t y1, uint8_t y2)
{
  ...
  if ( y1 < dog_min_y )
  {
    ...
    else
    {
      ...
      tmp = dog_pgm_read(dog_get_mask0()+tmp);
    }
  }
  else
  {
    if ( y2 >= dog_max_y )
    {
      ...
      tmp = y1;
      tmp &= (unsigned char)7;
      tmp = dog_pgm_read(dog_get_mask7()+tmp);
    }
    else
    {
      ...
      t2 = y2;
      t2 &= (unsigned char)7;
      t2= dog_pgm_read(dog_get_mask0()+t2);
      tmp &= t2;
    }
  }
  return tmp;
