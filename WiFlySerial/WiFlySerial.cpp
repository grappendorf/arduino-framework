...
const char* WiFlyFixedPrompts[N_PROMPTS] = { "","AOK","CMD","ERR: ?", "",">","*CLOS*","*OPEN*" };
uint16_t WiFlyFixedFlags[N_PROMPTS] = {PROMPT_EXPECTED_TOKEN_FOUND, PROMPT_AOK, PROMPT_CMD_MODE, PROMPT_CMD_ERR, PROMPT_READY,    	PROMPT_READY,PROMPT_CLOSE, PROMPT_OPEN};
...
//    sIP.substring(posStart,posEnd).toCharArray(alphabuf,posEnd-posStart+1);
...
//  PString IPString(pBuffer, buflen);
//
//  for (int i = 0;  i<UC_N_IP_BYTES; i++) {
//    IPString.print((int)pIP[i]);
...
//  char szIndicator[INDICATOR_BUFFER_SIZE];
  ...
  bStart = SendCommand( GetBuffer_P(STI_WIFLYDEVICE_SET_UART_MODE, szCmd, SMALL_COMMAND_BUFFER_SIZE),
                         (char *)WiFlyFixedPrompts[WIFLY_MSG_AOK],
                         szResponse,
                         COMMAND_BUFFER_SIZE );
...
boolean WiFlySerial::startCommandMode(char* pBuffer, const int bufSize) {
  ...
  while  (!bWiFlyInCommandMode || bWaiting ) {
      ...
      if ( ( millis() - TimeAtStart) >= ATTN_WAIT_TIME) {
        bWaiting = false;
      }
      nTries++;
...
char* WiFlySerial::showNetworkScan( char* pNetScan, const int buflen) {
  ...
  SendCommand((char *)"scan", (char *)"'", pNetScan, buflen, true, JOIN_WAIT_TIME, true, false) ;
...
boolean WiFlySerial::setProtocol( unsigned int iProtocol) {
  ...
  iTmp = iProtocol & WIFLY_IPMODE_UDP;
  if (iTmp ) {
      iMode |= 0x01;
  }
  ...
  bOk = SendCommandSimple(szCommand , (char *)WiFlyFixedPrompts[WIFLY_MSG_AOK]);
  reboot();
...
boolean WiFlySerial::openConnection(const char* pURL, const unsigned long iWaitTime) {
  ...
  bWiFlyConnectionOpen = SendCommand(bufCommand,(char *)WiFlyFixedPrompts[WIFLY_MSG_OPEN], bufOpen, INDICATOR_BUFFER_SIZE, false, iWaitTime , true, false);
...
boolean WiFlySerial::closeConnection() {
  ...
  return SendCommand(GetBuffer_P(STI_WIFLYDEVICE_CLOSE, bufCmd, INDICATOR_BUFFER_SIZE),(char *)WiFlyFixedPrompts[WIFLY_MSG_CLOSE], bufClose, COMMAND_BUFFER_SIZE, false, 2000);
...
//  char szIndicator[INDICATOR_BUFFER_SIZE];
  ...
  return SendCommand( szBuffer, (char *)WiFlyFixedPrompts[WIFLY_MSG_AOK], szReply,  INDICATOR_BUFFER_SIZE, true );
}
...
void WiFlySerial::reboot() {
  ...
  if (!SendCommandSimple( szCommand ,   (char *)WiFlyFixedPrompts[WIFLY_MSG_AOK] )) {
